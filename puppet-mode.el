...
(defvar puppet-font-lock-keywords
  (list
   ...
   (list (concat "ensure[ \t\n]*=>[ \t\n]*"
                 (regexp-opt '("absent" "configured" "defined" "directory"
                               "false" "file" "held" "installed" "latest"
                               "link" "mounted" "no_shutdown" "present"
                               "purged" "role" "running" "shutdown" "stopped"
                               "true" "unmounted")
                             'symbols))
   ...
   ;; keywords (important)
   (list (regexp-opt
          '("alert" "crit" "debug" "emerg" "err" "fail" "info" "notice"
            "warning")
          'symbols)
         0 'font-lock-warning-face)
   ;; keywords (normal)
   (list (regexp-opt
          '("and" "case" "class" "contain" "create_resources" "default"
            "define" "defined" "each" "else" "elsif" "epp" "err" "extlookup"
            "false" "file" "filebucket" "filter" "fqdn_rand" "generate"
            "hiera" "hiera_array" "hiera_hash" "hiera_include" "if" "import"
            "in" "include" "inherits" "inline_epp" "inline_template" "lookup"
            "map" "md5" "node" "or" "realize" "reduce" "regsubst" "require"
            "search" "sha1" "shellquote" "slice" "split" "sprintf" "tag"
   ...
   (list (regexp-opt
          '("abs" "any2array" "assert_private" "base64" "basename" "bool2num"
            "bool2str" "capitalize" "ceiling" "chomp" "chop" "clamp" "concat"
            "convert_base" "count" "defined_with_params" "delete" "delete_at"
            "delete_undef_values" "delete_values" "difference" "dirname"
            "dos2unix" "downcase" "empty" "ensure_packages" "ensure_resource"
            "flatten" "floor" "fqdn_rand_string" "fqdn_rotate"
            "get_module_path" "getparam" "getvar" "grep" "has_interface_with"
            "has_ip_address" "has_ip_network" "has_key" "hash" "intersection"
            "is_a" "is_absolute_path" "is_array" "is_bool" "is_domain_name"
            "is_float" "is_function_available" "is_hash" "is_integer"
            "is_ip_address" "is_mac_address" "is_numeric" "is_string" "join"
            "join_keys_to_values" "keys" "load_module_metadata" "loadyaml"
            "lstrip" "max" "member" "merge" "min" "num2bool" "parsejson"
            "parseyaml" "pick" "pick_default" "prefix" "pw_hash" "range"
            "reject" "reverse" "rstrip" "seeded_rand" "shuffle" "size" "sort"
            "squeeze" "str2bool" "str2saltedsha512" "strftime" "strip"
            "suffix" "swapcase" "time" "to_bytes" "try_get_value" "type3x"
            "type_of" "union" "unique" "unix2dos" "upcase" "uriescape"
            "validate_absolute_path" "validate_array" "validate_augeas"
            "validate_bool" "validate_cmd" "validate_hash" "validate_integer"
            "validate_ip_address" "validate_numeric" "validate_re"
            "validate_slength" "validate_string" "validate_x509_rsa_key_pair"
